...
https://grocery-assistants.web.app/
Link to the GitHub: 
	https://github.com/UWcse482a-2020/GroceryAssistants
Testing:
For testing search, we currently need to use UPC codes instead of string names. 
Please test using this UPC code instead of a product name: 11110001320

User manual
The following steps detail how one can navigate, test, and otherwise interact with the deployed version of eShoppingAssistant. If desired, please feel free to set up a local copy. In that case, please first see Instructions to clone and fork a local version of eShoppingAssistant on page 4.
Visit this website: https://grocery-assistants.web.app/
Select allow for your location as indicated below. 

Note: without enabling location, the app is not be able to provide meaningful data
Click on the green “Search for a Product” button

Using the search bar, enter a UPC code to loop up an item. We recommend using this UPC code for bread 11110001320. You could find a full list of UPC code for WIC-eligible food here. 
	
Press the search button

If desired, update the quantity of the product you have just searched by first entering the new quantity into the box then selecting “update”. Next time when you search for the product, the quantity will be updated and stores will be re-ranked for recommendations. 
Some different use cases entail updating a product quantity as described in step 6 or searching for a product as described in steps 4 and 5. 
The only failure state that we address is if someone searches for a product that is not in our database. In that case, we show an error message like this:

 
Instructions to clone and fork a local version of eShoppingAssistant
Git: Go to https://github.com/UWcse482a-2020/GroceryAssistants and get the link for cloning or forking. Run
git clone <repo_link>
to clone a copy of the repo.
Opening / Editing: You should be able to open the website by opening index.html at the top of the repo directory right away. If you would like to host a local server (and see real time changes) and have Python installed, you could run
python -m http.server
at the root directory and open the prompted link. The main logic of our app is implemented in main.js, and the content (images, font, etc.) is in assets. 
Deployment details: We used Firebase by Google to host a real-time readable/writable database as well as our website itself. The deployment content are copies of the website in public used for deployment. To use Firebase for deployment, we followed the steps provided here and run:
firebase init -> firebase serve -> firebase deploy
	The end result is at https://grocery-assistants.web.app/
We’d be happy to add course staff to our Firebase project and explain how the database was structured if needed. Please let us know! 
